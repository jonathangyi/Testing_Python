...
with tab1:
    ...
    with col2:
        ...
        if not stock_df.empty and not btc_df.empty:
            # Ensure single-level indices before merging
            btc_df = btc_df.reset_index(drop=True)
            stock_df = stock_df.reset_index(drop=True)
        ...
        for _, event in world_events.iterrows():
            ...
            st.markdown(f"**{event['date']}**: {event['event']} <span style='color:{impact_color};'>({event['impact']:+.1f})</span>",# filepath: c:\Users\james\OneDrive\Desktop\Crypto\Testing_Python\btc_dashboard.py
import streamlit as st
import pandas as pd
import numpy as np
import requests
